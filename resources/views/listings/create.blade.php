@extends('layout')
@section('content')
<x-card class="border-gray-200 p-10 max-w-lg mx-auto mt-24">
    ...
    <form method="POST" action="{{url('lisiting')}}" enctype="multipart/form-data">
        @csrf
        <div class="mb-6">
        ...
        <div class="mb-6">
            ...
            <textarea class="border border-gray-200 rounded p-2 w-full" name="description" rows="10"
                placeholder="Include tasks, requirements, salary, etc">{{old('tags')}}</textarea>
            @error('description')
            <p class="text-red-500 text-sm">{{$message}}</p>
            @enderror
        </div>
